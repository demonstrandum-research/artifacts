...
const NSHELL: usize = 109; // |p - c|^2 in 0..=108
const HIST_P: usize = 20; // pair-count histogram bins
const HIST_T: usize = 40; // total-point-count histogram bins

#[inline]
fn mirror(p: Point) -> Point {
    [12 - p[0], 12 - p[1], 12 - p[2]]
}

#[inline]
fn shell(p: Point) -> usize {
    let d = |v: i32| ((v - 6) * (v - 6)) as usize;
    d(p[0]) + d(p[1]) + d(p[2])
}

/// Orbit representatives: the 1098 cells with cell_index(p) < cell_index(sigma p).
/// The fixed point (6,6,6) is excluded automatically.
...
impl Global {
    ...
    /// Archive a distinct >= 34-point set (worker pre-deduplicates by hash);
    /// every pooled set is independently brute-force re-verified first.
    fn pool_34(&self, pts: &[Point]) {
        assert!(
            find_zero_5subset(pts).is_none(),
            "BUG: >=34-point pool candidate failed brute-force validation"
        );
        self.sets34_pooled.fetch_add(1, Relaxed);
        self.pool_append("pool_34.jsonl", pts, |b| &mut b.pool34_lines);
    }
...
impl<'g> Worker<'g> {
    ...
    fn extend_stat(&mut self) -> usize {
        ...
        if !added.is_empty() {
            ...
            if total >= 33 {
                self.g.pool_append("pool_total33.jsonl", self.st.points(), |b| {
                    &mut b.pool33_lines
                });
            }
            if total >= 34 {
    ...
    fn note_locopt(&mut self) {
        ...
        if np == 16 {
            // archive distinct 32-point symmetric optima for later seeding
            let h = set_hash(self.st.points());
            if h != self.last_pool_hash {
                self.last_pool_hash = h;
                self.g.pool_append("pool_16p.jsonl", self.st.points(), |b| &mut b.pool16_lines);
            }
        } else if np >= 17 {
            // every distinct symmetric >= 34-point set is a record set: archive
            let h = set_hash(self.st.points());
...
fn main() {
    ...
    let cfg = Cfg { stall, extmin };
    let reps = orbit_reps();
    assert_eq!(reps.len(), (NCELLS - 1) / 2); // 1098 orbit-pairs, center excluded
    println!(
        "symsearch: central-inversion symmetric ILS | n={N} reps={} threads={threads} secs={secs} seed={seed:#x} stall={stall} extmin={extmin} target={target}",
        reps.len()
    );
    let t0 = Instant::now();
    std::thread::scope(|sc| {
...
mod tests {
    ...
    fn equal_shell_pairs_are_degenerate() {
        let p: Point = [6, 6, 0];
        let q: Point = [6, 0, 6]; // both shell 36
    ...
    fn mini_ils_run_is_exactly_valid() {
        ...
        let g = Global::new(dir, 1, 35);
        let cfg = Cfg { stall: 10_000, extmin: 12 };
        let reps = orbit_reps();
